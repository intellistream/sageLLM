...
class ExecutionCoordinator:
    ...
    async def execute_request(
        ...
        instance.current_load = min(
            1.0,
            instance.active_requests / instance.max_concurrent_requests
        )
    ...
    def _update_metrics(
            ...
            if request.latency_ms:
                self.metrics.avg_latency_ms = (
                    (self.metrics.avg_latency_ms * (self.metrics.completed_requests - 1) +
                     request.latency_ms) / self.metrics.completed_requests
                )
        else:
        ...
        if self.metrics.completed_requests > 0:
            self.metrics.slo_compliance_rate = (
                1.0 - (self.metrics.slo_violations / self.metrics.completed_requests)
            )
    ...
    async def scale_instances(self, target_count: int):
        ...
        if target_count > current_count:
            ...
            logger.info(f"Scaling down from {current_count} to {target_count} instances")
