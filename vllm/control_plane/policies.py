...
class PriorityPolicy(SchedulingPolicy):
    ...
    def prioritize(self, requests: List[RequestMetadata]) -> List[RequestMetadata]:
        ...
        return sorted(
            requests,
            key=lambda r: (r.priority.value, r.arrival_time)
        )
...
class SLOAwarePolicy(SchedulingPolicy):
    ...
    def prioritize(self, requests: List[RequestMetadata]) -> List[RequestMetadata]:
        ...
        return sorted(
            requests,
            key=lambda r: self._calculate_urgency(r),
            reverse=True
        )
...
class CostOptimizedPolicy(SchedulingPolicy):
    ...
    def prioritize(self, requests: List[RequestMetadata]) -> List[RequestMetadata]:
        ...
        return sorted(
            requests,
            key=lambda r: (r.priority.value, r.cost_budget or float('inf'))
        )
    ...
    def _estimate_cost(
        ...
        gpu_cost_per_sec = (
            self.price_per_gpu_hour * instance.gpu_count / 3600
        )
